...
class LunarLander(gym.Env, EzPickle):
    ...
    def set_state(self, x, y, angle, angular_velocity=.0, linear_velocity=(.0, .0), center_force=(.0, .0)):
        # legs
        for leg in self.legs:
            leg.position = (
                leg.position[0] + x - self.lander.position[0],
                leg.position[1] + y - self.lander.position[1],
            )
        # lander
        self.lander.position = (x, y)
